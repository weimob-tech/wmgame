...
(function (wmGame) {
    ...
    function addEventListener(mc,type,onCompvare,type2,shuX){
        if(type2==undefined) type2=1;
        if(type2==1&&shuX==undefined){
            shuX={x:mc.x,y:mc.y,width:mc.width,height:mc.height};
        }
        mc._eventType=type2;
        mc._eventSX=shuX;
        var event={
    ...
    function removeEventListener(_this,type){
        ...
        if(type=="touchstart"){
            removeEventListener(_this,"mousedown");
        }else if(type=="touchmove"){
            removeEventListener(_this,"mousemove");
        }else if(type=="touchend"){
            removeEventListener(_this,"mouseup");
            removeEventListener(_this,"mouseout");
        }
    }
    ...
    function inite(){
        ...
        scg.inite(cid,_that2.enba,_that2.enbaFW);
        ...
        var scg={
            inite:function (id,enba,enbaFW) {
                ...
                if(istouch=="touchstart"){
                    scg.canvas.addEventListener("touchstart",eventDown);
                    scg.canvas.addEventListener("touchmove",eventDown);
                    scg.canvas.addEventListener("touchend",eventDown);
                    scg.canvas.addEventListener("touchcancel",eventDown);
                }else if(istouch=="click"){
                    scg.canvas.addEventListener("mousemove",eventDown2);
                    scg.canvas.addEventListener("mousedown",eventDown2);
                    scg.canvas.addEventListener("mouseup",eventDown2);
                    scg.canvas.addEventListener("mouseout",eventDown2);
                }

                scg.remove=function () {
                    if(istouch=="touchstart") {
                        scg.canvas.removeEventListener("touchstart", eventDown);
                        scg.canvas.removeEventListener("touchmove", eventDown);
                        scg.canvas.removeEventListener("touchend", eventDown);
                        scg.canvas.removeEventListener("touchcancel", eventDown);
                    }else if(istouch=="click") {
                        scg.canvas.removeEventListener("mousemove", eventDown2);
                        scg.canvas.removeEventListener("mousedown", eventDown2);
                        scg.canvas.removeEventListener("mouseup", eventDown2);
                        scg.canvas.removeEventListener("mouseout", eventDown2);
                    }

                };

                function eventDown(e) {
                    scg.events(e,function (p) {
                        enbafwFun(e,p);
                    });
                }

                function eventDown2(e) {
                    e.changedTouches=[{}];
                ...
                function enbafwFun(e,p) {
                    if(enba){
                        if(enbaFW==undefined||(enbaFW!=undefined&&(p.x>=enbaFW.x&&p.x<=(enbaFW.x+enbaFW.width)&&p.y>=enbaFW.y&&p.y<=(enbaFW.y+enbaFW.height)))){
                            e.stopPropagation();
                            e.preventDefault();
                        }
                    }
            ...
            openRepick:function (opening,noClear) {
                ...
                scg.onFrameHandle=function (){
                    ...
                    if(scg._time==undefined) scg._time=window.requestAnimationFrame(onFrameHandle);
                ...
                function onFrameHandle() {
                    ...
                    if((t1-t0)<tjg*fps){
                        ...
                        if(scg.opening) scg._time=window.requestAnimationFrame(onFrameHandle);
                ...
                function addEvent(mc) {
                    if(mc._eventType==undefined||mc._event==undefined) return;
                        ...
                        if(ctx.isPointInPath(mc._event.x,mc._event.y)){
            ...
            removeTime:function(){
                ...
                if(scg._time!=undefined){
                    window.cancelAnimationFrame(scg._time);
                    scg._time=undefined;
                }
            ...
            events:function (e,fun) {
                ...
                var  bl=scg.canvas.width/scg.canvas.getBoundingClientRect().width;
                var a={l:scg.canvas.getBoundingClientRect().left+window.scrollX,t:scg.canvas.getBoundingClientRect().top+window.scrollY};
                var _x=(e.changedTouches[0].pageX-a.l)*bl;
                var _y=(e.changedTouches[0].pageY-a.t)*bl;
                if(fun!=undefined){
                    fun({x:_x,y:_y});
                }
                for(var i=0;i<events.length;i++){
                    if(events[i].type==e.type){
                        events[i].mc._event={x: _x,y:_y,fun:events[i].fun,mc:events[i].mc};
                    }
                }
                function getAlloffsetParent(e,a,c,d) {
                    if(c==undefined||c==1){
                        a.l += e.offsetLeft;
                        a.t += e.offsetTop;
                    }
                    ...
                    //     a.t -= e.scrollTop;
                    // }
                    if(e.parentElement!=null&&(c==undefined||c==2)) {
                        getAlloffsetParent(e.parentElement, a);
                    }
                    // if(e.offsetParent!=null&&(c==undefined||c==1)) {
                    //     getAlloffsetParent(e.offsetParent, a,1,d);
                    // }
                }
